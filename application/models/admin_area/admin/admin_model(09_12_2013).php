...
class Admin_model extends CI_Model {
    ...
    function __construct() {
     ...
     function create_instance_account($instancename,$instanceyear,$instance_academic_year,$instanceterm) {

        $query = $this->db->query("SELECT instance_name FROM instance_details WHERE instance_name = '" . $instancename . "' ");

        if ($query->num_rows() < 1) {


           $result = $this->db->query("INSERT INTO instance_details(instance_name,year,academic_year,term,flag)
                VALUES ('" . $instancename . "','" . $instanceyear . "','" . $instance_academic_year . "','" . $instanceterm . "', '1')");
     ...
     /*function create_instance_account($instancename) {
        ...
        if ($query->num_rows() < 1) {
            ...
            $result = $this->db->query("INSERT INTO instance_details(instance_name) VALUES ('" . $instancename . "')");
    ...
    /* function get_school() 
      {
      $query = $this->db->query("select distinct(`school_name`) from student_details where school_name!=''");
      return $query->result();
      } */

    function get_schools() {

        $query = $this->db->query("SELECT   school_name FROM student_details where school_name!=''");
        $schools = array();
	...
	function login_validate($adminUserName = NULL, $adminPassword = NULL) {

        $query = $this->db->query("SELECT a.admin_id, a.admin_username, a.admin_role_id, a.admin_password FROM ls_admin AS a INNER JOIN ls_roles AS r ON a.admin_role_id = r.role_id WHERE admin_username = '" . $adminUserName . "' and admin_password = '" . $adminPassword . "'");

        if (($query->num_rows() == 1)) {
            //entering last login time
            $this->db->query("UPDATE ls_admin SET last_login = now() WHERE admin_username = '" . $adminUserName . "' AND admin_password = '" . $adminPassword . "'");

            $db_row = $query->row();

            $this->admin_id = $db_row->admin_id;
            $this->admin_username = $db_row->admin_username;
            $this->admin_pwd = $db_row->admin_password;
            $this->admin_role_id = $db_row->admin_role_id;
    ...
    /*function create_user_account($user_email, $username, $user_password, $userrole, $assignschools, $admin_id, $admin_email, $admin_role_id) {

       // print_r($assignschools);



        $query = $this->db->query("SELECT user_email,user_name FROM user_detail WHERE user_password = '" . $user_password . "' AND user_name = '" . $username . "' ");
        ...
        if ($query->num_rows() < 1) {
            ...
            $toInsert = '';
            foreach ($assignschools as $p) {
                $toInsert = $toInsert . ',' . $p;
            }
            //foreach ($assignschools as $item_id=>$item_qty){

            $result = $this->db->query("INSERT INTO user_detail(user_email,user_name,user_password,user_role,assign_school) VALUES ('" . $user_email . "','" . $username . "',
                            '" . $user_password . "','" . $userrole . "','" . $toInsert . "')");
	...
	function create_user_account($user_email, $username, $user_password, $userrole, $assignschools, $admin_id, $admin_email, $admin_role_id) {
     ...
     $queryAdmin = $this->db->query("SELECT admin_username,admin_password FROM ls_admin WHERE admin_password = '" . $user_password . "' AND admin_username = '" . $username . "' ");
        $query = $this->db->query("SELECT user_email,user_name FROM user_detail WHERE user_password = '" . $user_password . "' AND user_name = '" . $username . "' ");

        if (($query->num_rows() < 1) && ($queryAdmin->num_rows() < 1 )){
            ...
            $result = $this->db->query("INSERT INTO ls_admin(admin_username,admin_password,admin_role_id) VALUES ('" . $username . "',
