...
class Dataentry_model extends CI_Model {
     ...
     */
	 ...
	 function get_schools($instance_id_value,$userID) {

        $schools = array();
        $query_user = $this->db->query("SELECT  assign_school  FROM user_detail WHERE id = $userID ");
       ...
       // $instance_id_value = '1';
        ...
        foreach ($schoolslist as $school) {
            //echo "SELECT distinct(school_name)  FROM student_details WHERE school_name ='$school' and  instance_id = '{$instance_id_value}'";
            $query = $this->db->query("SELECT distinct(school_name)  FROM student_details WHERE school_name ='$school' and  instance_id = '{$instance_id_value}'");
            //print_r($query->result());
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $sch) {
                    //$students1[$student->first_name] = $student->first_name;

                    $schoolsname = $sch->school_name;
                }
            } else {
                $schoolsname = '';
            }
            if ($schoolsname != '') {
                $schools[$school] = $schoolsname;
            }
        }
  ...
  /* function get_instance_data($userID) {
        ...
        if ($query_user->num_rows() == 1) {
            ...
            $row = $query_user->row();
            $assignInstances = $row->assign_school;
        }
        $assignInstanceslist = substr($assignInstances, 1);



        $Instancelist = explode(",", $assignInstanceslist);
       // $instance_id_value = '1';

        foreach ($Instancelist as $instance) {
     //echo "SELECT distinct(school_name)  FROM student_details WHERE school_name ='$school' and  instance_id = '{$instance_id_value}' order by school_name asc";
            $query = $this->db->query("SELECT distinct(instance_name),instance_id  FROM instance_details WHERE instance_id ='$instance' order by instance_name asc");
            //print_r($query->result());
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $sch) {
                    //$students1[$student->first_name] = $student->first_name;
            ...
            if ($instancesname != '') {
                $instances[$instance] = $instancesname;
            }
        }

       // print_r($instances);exit();
        return $instances;
    }
  */
    
    /*function get_schools($instance_id_value) {
        // echo $schoolname;

        $query = $this->db->query("SELECT distinct(school_name)  FROM student_details WHERE instance_id ='$instance_id_value'  order by school_name asc");
        //print_r($query->result());
        $schoolsname = array();

        if ($query->result()) {
            foreach ($query->result() as $school) {
                $schoolsname[$school->school_name] = $school->school_name;
            }
            return $schoolsname;
        } else {
            return FALSE;
...
function get_status_instance_list($instance_id_value,$school, $preclasses) {
    ...
    function search_student_list($school, $enrollstatus, $preclass) {
        ...
        $query = $this->db->query("SELECT * FROM student_details 
						WHERE school_name='$school' AND primary_classroom='$preclass' AND enrollment_status='$enrollstatus' order by child_id asc");
