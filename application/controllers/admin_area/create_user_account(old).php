...
class Create_user_account extends CI_Controller {
   ...
   {	
		...
		$this->form_validation->set_rules('userEmail', 'Email', 'trim|required|valid_email|max_length[150]|is_unique[cr_users.user_email]|xss_clean');
		 
		if($this->form_validation->run() == FALSE) {		
			
			 $this->admin_template->write_view('content', 'admin_area/create_user_account/create_user_account_view');
			...
			// Validate the user 
			$result = $this->admin_model->create_user_account($user_email, $user_name, $user_password,$user_key_val,$admin_id,$admin_email,$admin_role_id);
			...
			if($result == TRUE) {
				...
				$this->email->from('selvem.jose@gmail.com', 'Credit Inspire');
				...
				if($this->email->send()) {
					...
					$this->session->set_flashdata('success_message', 'CSR account has been created successfully.');
					redirect('admin_area/dashboard');	
				} else {
					//echo $this->email->print_debugger();
					$this->session->set_flashdata('error_message', 'Oops!!! Some error occured while sending mail, Please try again.');
					redirect('admin_area/create_user_account');
				}	
				
			 } else {			
				$this->session->set_flashdata('error_message', 'Oops!!! Some error occured while creating an Account, Please try again.');
				redirect('admin_area/create_user_account');
			}				 
		}	  
   }
