...
class Data_entry extends CI_Controller {
    ...
    function getchronological_age() {

        $get_startdate = $this->input->post('startdate');
        $get_enddate = $this->input->post('enddate');

        $d1 = new DateTime($get_startdate);
        $d2 = new DateTime($get_enddate);
        ...
        $display_date = $yearDiff . '--' . $monthdiff . ':' . "testdate";
...
//print_r($_POST);
        ...
        for ($i = 0; $i <= $recordcount; $i++) {
            ...
            $updateArray[] = array(
				...
				'chronological_age' => $get_student_data["input_chronological_age_{$i}"],
