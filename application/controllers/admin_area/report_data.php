...
class Report_data extends CI_Controller {
    ...
    //$this->load->model('admin_area/export/export_model');
        //$this->load->model('admin_area/report/report_model');
        $this->load->library('admin_popup_template');
        
         //$this->load->model('services_json_model');
                // $this->load->library('JSON');
        //$this->load->library('JSON');
        //$this->load->library('jqgrid_dist');
        
        //$this->load->library('inc/jqgrid_dist');
       // $this->load->library('inc');
        ...
        $this->admin_template->write('title', 'Admin Little Sprouts - Report Data');
...
function index() {
        redirect('admin_area/report_data/getschool_list');
    }
    ...
    function getschool_list() {
       ...
       $out = array();
       if($_POST){
           $get_school_data = ($_POST);
            $out['schoolsname'] = $get_school_data['school'];
           
       }
        //$out['schools'] = $this->export_model->get_schools();
        $this->admin_template->write_view('content', 'admin_area/reports/report_data_view', $out, TRUE);
        $this->admin_template->render();
         
    }
    /* function get_school_grid_data() { 
    
          $data = array();
          $get_school_data = ($_POST);
      $data['schoolsname'] = $get_school_data['school'];
      $data['schools'] = $this->export_model->get_schools();
       $this->admin_template->write_view('content', 'admin_area/reports/report_data_view', $data, TRUE);
        $this->admin_template->render();
